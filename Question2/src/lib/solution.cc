...
void BST::InorderRec(TreeNode *root,vector<int> &result){
  if(root == NULL){
    return ;
  }
...
vector<int> BST::InoderPrint(){
  InorderRec(root_,result);
  return result;
}

vector<int> BST::InorderNonRec(TreeNode *root){
  vector<int> v = {};
  ...
  while(curr != nullptr || !(output.empty())){
    while(curr != nullptr){
      output.push(curr);
      curr = curr -> left;
    }
...
vector<int> BST::InoderPrint2(){
  return InorderNonRec(root_);
}